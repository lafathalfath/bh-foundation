        ...
        <section class="py-20 ">
                ...
                <form method="GET" action="{{ route('allnews') }}" class="flex flex-wrap items-center gap-4">
                    ...
                    <div class="relative w-full sm:w-64">
                        <select
                            ...
                            <option value="programs" {{ $type === 'programs' ? 'selected' : '' }}>@lang('messages.programs')</option>
                            <option value="scholarship" {{ $type === 'scholarship' ? 'selected' : '' }}>@lang('messages.scholarship')</option>
                        </select>
            ...
            <div class="flex items-center justify-center">
                <div class="flex items-center justify-center gap-2">
                    <a @if (!$first)
                        href="{{ route('allnews', ['page' => $current-1, 'type' => request('type')]) }}"
                    @endif class="px-2.5 pb-1 text-white font-bold hover:scale-125 {{ $first ? 'bg-gray-400' : "rounded bg-[$settings->primary_color]" }}">«</a>
                    @if ($startpage > 1)
                        <div>...</div>
                    @endif
                    @for ($i = $startpage; $i <= $endpage; $i++)
                        <a href="{{ $current == $i ? route('allnews', ['page' => $i, 'type' => request('type')]) : '' }}" class="px-1 rounded {{ $current == $i ? "border-x-2 border-[$settings->primary_color]" : "hover:bg-[$settings->primary_color]/[0.5] hover:text-white" }}">{{ $i }}</a>
                    @endfor
                    @if ($endpage > $lastpage)
                        <div>...</div>
                    @endif
